...
import { createGrading, type CallModel, type GradingDeps } from '@/server/grading/operational';
...
test('수용시험 3 — B (1,5,3)·(5,1,3)은 총점이 같아도 3차 호출 후 중앙값 (3,3,3)·50점', async () => {
  ...
  const run = await createGrading(h.deps).runOperationalScoring(request({ band: 'B' }));
