...
import { produceFeedback, feedbackNotRequested, type FeedbackDraft } from '@/lib/feedback';
import type {
  CallRecord,
...
import type { RegistryApi } from '@/server/registry/contract';
...
export function createGrading(deps: GradingDeps): GradingApi {
  ...
  return {
    async runOperationalScoring(req: GradingRequest): Promise<ScoringRun> {
      ...
      const cues = commonOnly ? null : deps.registry.getCues(req.questionId);
      ...
      const finish = (
        ...
        return {
          ...
          rubricVersion: entry.rubricVersion || RUBRIC_VERSION,
      ...
      if (pii.decision === 'hold_for_teacher') {
        ...
        const holdRecord: CallRecord = {
          callId: `${req.operationId}:hold`,
          retryIndex: 0,
          levels: null,
          // 탐지된 유형만 남기고 원문 조각은 남기지 않는다.
          failureReason: `privacy_hold_for_teacher: ${pii.matchedTypes.join(',')}`,
          startedAt: t,
          finishedAt: t,
          durationMs: 0,
        };
        return missing('required_call_failed', [holdRecord]);
      }
